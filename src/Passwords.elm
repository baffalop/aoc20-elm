...
validPasswords tagger validate =
    parseInput tagger >> List.filter (validate >> (==) Valid)
...
validateOldPassword : OldPassword -> Validity
validateOldPassword { min, max, char, pwd } =
    let
        count =
            countChar char pwd
    in
...
countChar char =
    String.filter ((==) char) >> String.length


validateNewPassword : NewPassword -> Validity
validateNewPassword { pos1, pos2, char, pwd } =
    let
        indices =
            String.indices (String.fromList [ char ]) <| String.cons '.' pwd
    in
    if List.member pos1 indices |> xor (List.member pos2 indices) then
        Valid
...
parseInput : Tagger password -> String -> List password
parseInput tagger =
    String.lines
        >> List.filterMap
            (P.run (passwordParser tagger)
                >> Result.toMaybe
            )


passwordParser : Tagger password -> Parser password
passwordParser tagger =
    P.succeed tagger
        |= P.int
        |. P.symbol "-"
        |= P.int
